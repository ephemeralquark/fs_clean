...
from src.input.hasher import Hasher
...
class TestFileMd5:
    ...
    def test_empty_file(self, tmp_path: Path):
        f = tmp_path / "empty"
        f.touch()
        assert Hasher().hash(f) == "d41d8cd98f00b204e9800998ecf8427e"

    def test_known_content(self, tmp_path: Path):
        f = tmp_path / "hello"
        f.write_text("hello", encoding="utf-8")
        assert Hasher().hash(f) == "5d41402abc4b2a76b9719d911017c592"
    ...
    def test_custom_buf_size(self, tmp_path: Path):
        ...
        h1 = Hasher().hash(f, buf_size=3)
        h2 = Hasher().hash(f, buf_size=65536)
        assert h1 == h2
