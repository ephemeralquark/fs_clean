...
class TestFindDuplicates:
    ...
    def test_no_duplicates(self, tmp_path: Path):
        files = [
            src.FileInfo(path=tmp_path / "a.txt", size=10, mtime=1.0),
            src.FileInfo(path=tmp_path / "b.txt", size=20, mtime=2.0),
            src.FileInfo(path=tmp_path / "c.txt", size=30, mtime=3.0),
        ]
        groups = src.DuplicationDetector().find_duplicates(files)
        assert groups == []
    ...
    def test_multiple_groups(self, tmp_path: Path):
        ...
        files = [
            src.FileInfo(path=tmp_path / "a.txt", size=6, mtime=1.0),
            src.FileInfo(path=tmp_path / "b.txt", size=6, mtime=2.0),
            src.FileInfo(path=tmp_path / "c.txt", size=6, mtime=3.0),
            src.FileInfo(path=tmp_path / "d.txt", size=6, mtime=4.0),
        ]
        groups = src.DuplicationDetector().find_duplicates(files)
        assert len(groups) == 2
    ...
    def test_three_way_duplicate(self, tmp_path: Path):
        ...
        files = [
            src.FileInfo(path=tmp_path / n, size=6, mtime=float(i))
    ...
    def test_oserror_during_hashing(self, tmp_path: Path):
        ...
        files = [
            src.FileInfo(path=tmp_path / "a.txt", size=4, mtime=1.0),
            src.FileInfo(path=tmp_path / "b.txt", size=4, mtime=2.0),
        ]
    ...
    def test_custom_hasher(self):
        ...
        detector = src.DuplicationDetector(hasher=mock_hasher)
        files = [
            src.FileInfo(path=Path("/x"), size=10, mtime=1.0),
            src.FileInfo(path=Path("/y"), size=10, mtime=2.0),
        ]
    ...
    def test_size_pre_filtering(self, tmp_path: Path):
        files = [
            src.FileInfo(path=tmp_path / "a.txt", size=5, mtime=1.0),
            src.FileInfo(path=tmp_path / "b.txt", size=5, mtime=2.0),
            src.FileInfo(path=tmp_path / "c.txt", size=100, mtime=3.0),
        ]
        groups = src.DuplicationDetector().find_duplicates(files)
...
class TestDuplicateGroup:
    ...
    def test_fields(self):
        group = src.DuplicateGroup(size=10, md5="abc", files=["a", "b"])
        assert group.size == 10
        assert group.md5 == "abc"
        assert group.files == ["a", "b"]  # namedtuple preserves original list

    def test_access_by_index(self):
        group = src.DuplicateGroup(size=10, md5="abc", files=[Path("/x")])
        assert group[0] == 10
        assert group[1] == "abc"
